...
+ (* operator is strength-reduced *)
  assert(OP:
     ...
     exists v',
        eval_operation ge (Vptr (fresh_block true sps) Ptrofs.zero) op' rs ## args' m = Some v' /\
        Val.lessdef v v').
  ...
  assert (EV'': exists v'', eval_operation ge (Vptr (fresh_block true sps) Ptrofs.zero) op' rs'##args' m' = Some v'' /\ Val.lessdef v' v'').
...
+ (* strength-reduced *)
  assert (ADDR:
     ...
     exists a',
        eval_addressing ge (Vptr (fresh_block true sps) Ptrofs.zero) addr' rs ## args' = Some a' /\
        Val.lessdef a a').
  ...
  intros (a'' & U & V).
  assert (W: eval_addressing tge (Vptr (fresh_block true sps) Ptrofs.zero) addr' rs'##args' = Some a'').
...
Opaque builtin_strength_reduction.
  ...
  assert (DFL: (fn_code (transf_function rm f))!pc = Some dfl ->
          exists (n2 : nat) (s2' : state),
            step tge
             (State s' (transf_function rm f) (Vptr (fresh_block true sps) Ptrofs.zero) pc rs' m'0) t s2' /\
            match_states n2
             (State s f (Vptr (fresh_block true sps) Ptrofs.zero) pc' (regmap_setres res vres rs) m') s2').
  {
...
- (* Icond, preserved *)
  ...
  intros EV1 TCODE.
  left; exists O; exists (State s' (transf_function (romem_for cu) f) (Vptr (fresh_block true sps) Ptrofs.zero) (if b then ifso else ifnot) rs' m'); split.
...
- (* Ijumptable *)
  ...
  left; exists O; exists (State s' (transf_function (romem_for cu) f) (Vptr (fresh_block true sps) Ptrofs.zero) pc' rs' m'); split.
